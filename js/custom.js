(function ($) {
    ...
    $(document).ready(function () {
       ...
       ============================*/
        ...
        Home Quote Form
        ============================*/
        $("#homeQuoteForm").on("submit", function (e) {
            e.preventDefault();
            ...
            const $form = $(form);
            const $button = $form.find(".home-quote-submit");
            const $message = $form.find(".home-form-message");
            const defaultText = $button.text();

            $message.removeClass("success").hide().text("");
            $button.prop("disabled", true).text("Submitting...");

            window.setTimeout(function () {
                form.reset();
                $form.find("select.select_option").niceSelect("update");
                $button.prop("disabled", false).text(defaultText);
                $message
                    .addClass("success")
                    .text("Successfully submitted. Thank you, we received your request.")
                    .fadeIn(160);
            }, 900);
        });
